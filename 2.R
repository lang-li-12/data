...
abline(h = 2000, col = "red")##剪切高度不确定，故无红线
...
datTraits = sample_trait[which(sample_trait$CGGA_ID %in% rownames(datExpr0)),]
CGGA_ID = datTraits$CGGA_ID
OS = datTraits$OS
datTraits = data.frame(CGGA_ID,OS)
datTraits = column_to_rownames(datTraits,var = 'CGGA_ID')
...
softPower = 7 #或者直接用预估值，即sft$powerEstimate
